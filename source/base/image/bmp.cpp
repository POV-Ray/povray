...
Image *Read (IStream *file, const ImageReadOptions& options)
{
    ...
    else
    {
        ...
        unsigned int pixelWidth = (has_alpha ? 4 : 3);
        unsigned int rowLength = file_width * pixelWidth;
        ...
        else 
        {
            for (int y = file_height - 1; y >= 0; y--)
            {
                ...
                {
                    int offset = x * pixelWidth;
                    SetEncodedRGBAValue(image, x, y, gamma, 255, rowBuffer[offset + 2], rowBuffer[offset + 1], rowBuffer[offset], a, premul);
                }
