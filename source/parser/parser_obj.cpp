...
void Parser::Parse_Obj (Mesh* mesh)
{
                ...
                CASE5 (CONCAT_TOKEN,STRUPR_TOKEN,STRLWR_TOKEN,DATETIME_TOKEN,STRING_ID_TOKEN)
                    ...
                    EXPECT_ONE
                        ...
                        CASE (MATERIAL_TOKEN)
                            ...
                            material.texture.SetCopy (povMaterial->Texture);
                ...
                if (strcmp (wordBuffer, "usemtl") == 0)
                {
                    ...
                    if (materialId == materialList.size())
                    {
                        ...
                        if (symbol == NULL)
                            ...
                            material.texture.SetCopy (*reinterpret_cast<TextureData*>(symbol->Data));
                        else if (symbol->Token_Number == MATERIAL_ID_TOKEN)
                            material.texture.SetCopy (reinterpret_cast<MATERIAL *>(symbol->Data)->Texture);
                        else
    ...
    if (!materialList.empty())
    {
        ...
        textureArray = reinterpret_cast<TextureData *>(POV_MALLOC(materialList.size() *sizeof(TextureData), "triangle mesh data"));
