...
*
* DESCRIPTION
*   A bit like object_pattern, with a cloud of points instead of just the
*   intersection point. The points in the clouds are weighted.
*
*  The cloud is generated by a compact packing of sphere inside a sphere, using 
*  Waterman distribution (spheres are at integer position of a latice, 
*  whenever x+y+z is odd)
*
*  x+y+z odd: means we ignore on purpose the intersection point 
*  (no sphere at <0,0,0> : inside would be really bogus at intersection)
*  Weight is 30 * sqrt(x^2+y^2+z^2) : far point weight more, on purpose.
* 
* CHANGES
*    2011-Mar-16 Creation
*    2011-Mar-20 Increase sphere diameter to avoid banding artefact
*
******************************************************************************/
typedef struct { signed char x,y,z; unsigned char w;} proximity_point;
#define PROXIMITY_CLOUD_SIZE 1264
static proximity_point  proximity_cloud[PROXIMITY_CLOUD_SIZE]={
{-1,-2,-8,249},
{1,-2,-8,249},
{-2,-1,-8,249},
{0,-1,-8,241},
...
//Max : 249.000000
//Min : 30.000000
// No need to compute it every time, just sum the weight once (we need a float/double, hence .0)
#define PROXIMITY_SUM_WEIGHT 239728.0
// Size of the containing sphere
#define PROXIMITY_RADIUS 8.3

DBL ProximityPattern::EvaluateRaw(const Vector3d& EPoint, const Intersection *, const Ray *, TraceThreadData *pThread) const
{
	if(pObject)
	{
    DBL ScaleRange = Range/PROXIMITY_RADIUS;
		Vector3d TPoint; // test point
		TRANSFORM Trans;
    // compute rotation to avoid banding/orientation issues
		Compute_Rotation_Transform(&Trans,EPoint);
	  unsigned int sum=0;
		for(unsigned int i=0;i<PROXIMITY_CLOUD_SIZE;i++)
		{
			TPoint = Vector3d( proximity_cloud[i].x, proximity_cloud[i].y, proximity_cloud[i].z );
			TPoint *= ScaleRange;
			MTransPoint(TPoint,TPoint,&Trans);
			TPoint += EPoint;
			if(pObject->Inside(TPoint,pThread))
				sum += proximity_cloud[i].w;
		}
		return sum/PROXIMITY_SUM_WEIGHT;
	}
...
{
    for(std::vector<ObjectPtr>::const_iterator Current_obj=vObject.begin();
        Current_obj!=vObject.end();
        ++Current_obj)
    {
          Destroy_Object(*Current_obj);
    }
}
BinaryPattern::BinaryPattern()
{
}
BinaryPattern::BinaryPattern(const BinaryPattern& obj):
    ContinuousPattern(obj)
{
    vObject.resize( obj.vObject.size());
    for(unsigned idx = 0;
        idx < vObject.size();
        ++idx)
    {
        vObject[idx] = Copy_Object(obj.vObject[idx]);
    }
}

ProportionPattern::ProportionPattern()
{
}

ProportionPattern::ProportionPattern(const ProportionPattern& obj):
    ContinuousPattern(obj)
{
    vObject.resize( obj.vObject.size());
    for(unsigned idx = 0;
...
*
* DESCRIPTION
*   A bit like object_pattern, for a collection of N objects.
*   For k-th object, if the point is not inside, value loose (1/2)^k
*   Range is [(1/2^k)..1], with a limit to 0..1
*
* CHANGES
*    2011-Aug-08 Creation
*
******************************************************************************/
DBL BinaryPattern::EvaluateRaw(const Vector3d& EPoint, const Intersection *, const Ray *, TraceThreadData *pThread) const
{
	DBL value=0.5;
	DBL sum=1.0;
	for(std::vector<ObjectPtr>::const_iterator Current_obj=vObject.begin();
			...
			Current_obj++)
	{
		if(!(*Current_obj)->Inside(EPoint, pThread))
			sum -= value;
		value /= 2.0;
	}
	return sum; 
...
* DESCRIPTION
*   A bit like object_pattern, for a collection of N objects.
*   For each object the point is inside, value gains +1/N
*   (in fact, +1, divides by N at the end)
*
* CHANGES
*    2011-Aug-08 Creation
*
******************************************************************************/
DBL ProportionPattern::EvaluateRaw(const Vector3d& EPoint, const Intersection *, const Ray *, TraceThreadData *pThread) const
{
	...
	if (counter)
		return (min(1.0,(1.0*sum)/(1.0*counter))); 
	else
		return 0.0;
}
...
*
* DESCRIPTION
*   Find the voronoi cell in which a point lies
*   For N cells, select the one with the smallest distance to its center
*   returned value is k/(N-1) , (k from 0 to N-1)
*
* CHANGES
*    2012-Oct-10 Creation
*
******************************************************************************/
DBL VoronoiPattern::EvaluateRaw(const Vector3d& EPoint, const Intersection *, const Ray *, TraceThreadData *) const
{
  unsigned int k=0;
  unsigned int n=0;
	DBL square=0.0;
  ...
  if (vPoint.empty())
      return 0.0;
  Vector3d focus(EPoint);
  std::vector<Vector3d>::const_iterator Current=vPoint.begin();
	best_square= (focus-(*Current)).lengthSqr();
  Current++;
  for(;
			Current!=vPoint.end();
			Current++)
	{
      n++;
      square= (*Current-focus).lengthSqr();
...
*
* CHANGES
*    2012-Nov-03 Creation
*
******************************************************************************/
DBL MasonryPattern::EvaluateRaw(const Vector3d& EPoint, const Intersection *pIsection, const Ray *pRay, TraceThreadData *pThread) const
{
	size_t k=0; /* nearest vertex */
	size_t m=0; /* second nearest vertex */
	DBL square=0.0;
	DBL best_square;
	DBL second_square;
	DBL ratio ;
	size_t maxi,current=0;
	Vector3d focus(EPoint);
	maxi = vCell.size();
  best_square=std::numeric_limits<DBL>::max();
  second_square=std::numeric_limits<DBL>::max();
  m= k = maxi;
	for(current=0;
			current<maxi;
			current++)
	{
		square= (focus-(vCell[current])).lengthSqr();
		if (square < best_square)
		{
			second_square = best_square;
			m = k;
			best_square = square;
			k = current;
		} 
	...
	if ( ( vValue[k] != vValue[m] ) 
			&&(Thickness > 0.0))
	{
		Vector3d dir((vCell[k])-(vCell[m]));
		Vector3d mid(((vCell[k])+(vCell[m]))/2);
		Vector3d vec(focus-mid);
		...
		{ 
			if (vec.lengthSqr()>0.0)
			{
				dir.normalize();
				//vec.normalize();
				ratio = abs(dot(dir,vec)/(Thickness));
				if (ratio <= 1.0)
				{
					return (ratio*(Middle))+((1.0-ratio)*(Border));     
				}
			}
			else 
			{
