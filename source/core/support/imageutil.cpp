...
TextureData material_map(const Vector3d& EPoint, const TextureData& Texture)
{
    ...
    {
        image_colour_at(dynamic_cast<ImagePattern*>(Texture.Pattern()->pattern.get())->pImage, xcoor, ycoor, colour, &reg_number); // TODO ALPHA - we should decide whether we prefer premultiplied or non-premultiplied alpha
