...
class BicubicPatch : public ObjectBase
{
	...
	protected:
		...
		bool intersect_subpatch(const Ray&, const TripleVector3d&, const DBL [3], const DBL [3], DBL *, Vector3d&, Vector3d&, DBL *, DBL *) const;
		static bool spherical_bounds_check(const Ray &, const Vector3d& c, DBL);
		...
		int bezier_subpatch_intersect(const Ray &, const ControlPoints *, DBL, DBL, DBL, DBL, IStack&);
		...
		int bezier_subdivider(const Ray &, const ControlPoints *, DBL, DBL, DBL, DBL, int, IStack&);
