...
namespace vfePlatform
{
    /**
        ...
        Inner workings:
            ...
            command line options are removed from the argument list.

        An instance of this class is part of vfeWinSession and can be accesses through
        vfeWinSession::GetWinConOptions().
    ...
    class WinConOptionsProcessor
    {
      public:
        ...
        struct WinPath
        {
            ...
            WinPath(const string &s, bool desc = false, bool wrt = false) : str(s), descend(desc), writable(wrt) { }
        };
        ...
        */
        ...
        WinConOptionsProcessor(vfeSession *session);
        virtual ~WinConOptionsProcessor() {} ;
     ...
     protected:
        ...
        string win_getcwd(void);
        string basename(const string &path);
        string dirname(const string &path);
        string win_readlink(const string &path);
        string pre_process_conf_line(const string &input);
        void add_permitted_path(list<WinPath> &paths, const string &input, const string &conf_name, unsigned long line_number);
