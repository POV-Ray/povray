...
# Colour bands to use
Bands = [[0,     0,     492.5, 492.5, "Blue",   x, (0,0,0), 0],
         [492.5, 492.5, 587.5, 587.5, "Green",  x, (0,0,0), 0],
         [587.5, 587.5, 999,   999,   "Red",    x, (0,0,0), 0]]
...
var('p')
f_D65 = rgb_D65 * p
f_D65_err = f_D65(p)-D65_internal
f_D65_errSqr = sum(f_D65_err(p).pairwise_product(f_D65_err(p)))
(D65_p) = minimize(f_D65_errSqr, [0])

pigment_D65_decompensation = rgb_D65
...
for it in test_list:
    rgb = vector(it)
    light = M_from_rgb * rgb
    pigment = light.pairwise_product(pigment_D65_compensation)
    print rgb, "->", pigment
